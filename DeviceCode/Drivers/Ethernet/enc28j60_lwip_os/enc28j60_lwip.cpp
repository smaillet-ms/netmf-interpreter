...
// by making these static it essentially means, despite
// all apperances to the contrary, that this driver only
// supports a single instance (e.g. you can't have more
// than one ENC28J60 in the system.)
...
static UINT8 s_receiveRetries = RECEIVE_RETRIES;
...
void enc28j60_handle_xmit_error( netif *pNetIF, SPI_CONFIGURATION  *SpiConf )
{
    UINT8 byteData;
...
// Context: ISR
// Remarks:
//    Disables the interrupt at the device then triggers
//    the continuation to handle pulling received packets
//    out of the NIC and posting them to the LWIP stack
void enc28j60_isr( GPIO_PIN Pin, BOOL PinState, void* pArg )
{
    ...
    // enable the continuation to retrieve data from the chip
    lwip_interrupt_continuation( );
}

/* ********************************************************************
   Interrupt handling routine on the main CLR thread as a continuation
   Technically this could be another thread as well, though there is the
   additional overhead of a stack for the thread.
   ******************************************************************** */
void enc28j60_lwip_interrupt( struct netif *pNetIF )
{
    ...
    */
    ...
    if( packetsLeft )
    {
        /* If there are more packets left re-queue this continuation handler */
        lwip_interrupt_continuation( );
    }
}
...
/* ********************************************************************
   ...
   from the interrupt continuation in the context of the main CLR thread.
...
bool enc28j60_lwip_setup_device( struct netif *pNetIF )
{
    ...
    {
        hal_printf( "enc28j60_lwip_setup_device: Wrong Device" );
    }
    ...
    {
        hal_printf( "enc28j60_lwip_setup_device: check duplex setting \r\n" );
    }
